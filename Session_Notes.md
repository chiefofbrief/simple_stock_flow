...
- **ADBE Specifics (Main Test):**
  ...
  - **Agreed Fixes:**
    - Increase News Reach: Update `news_alphavantage.py` to use `sort=RELEVANCE` or implement a multi-month fetch (10 items per month) to ensure 3-month coverage without increasing context weight.
    - Video Engagement: Implement a 5,000 view minimum threshold and include post dates in `tiktok.py` and `youtube.py` markdown output.
    - Master Date: Standardize "Generated" date formatting in `sentiment.py`.
  - **Under Investigation:**
    - Reddit Search: Transitioning from "Subreddit Scrape" (which missed low-volume tickers) to "Keyword Search". Need to verify SociaVault API documentation for the `/scrape/reddit/search` endpoint parameters before implementation.
    - Source Counting: Fixing logic in `news.py` to correctly identify unique sources from Perigon metadata.
    - News Source Value: Evaluating if both Perigon and AlphaVantage are needed or if FMP's Search Stock News API is a better alternative.
