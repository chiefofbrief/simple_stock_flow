...
def get_command(source, ticker, args):
    ...
    # Updated paths for WORKFLOW v1 structure
    base_path = "WORKFLOW v1/Scripts/Sentiment Scripts"
    cmds = {
        'news': ['python', f'{base_path}/news.py', ticker, '--markdown'],
        'reddit': ['python', f'{base_path}/reddit.py', '--ticker', ticker, '--markdown'],
        'tiktok': ['python', f'{base_path}/tiktok.py', ticker, '--markdown'],
        'youtube': ['python', f'{base_path}/youtube.py', ticker, '--markdown']
    }
...
def save_output(ticker, content):
    ...
    # Ensure shared_utils can be imported (same directory)
