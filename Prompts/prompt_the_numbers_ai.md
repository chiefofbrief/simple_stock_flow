...
**Data files**
- `Data/tickers/{TICKER}/{TICKER}_financial_analysis.md` — Full read. Primary input for Part A and Part B.
...
> Write the full analysis directly to `### The Numbers` in the Thesis file **as you generate it** — Part A metric by metric, then Part B, then the Central Question, Targeted Searches, Accounting Checklist, Accounting Analysis, and Hypothesis Check. Do **not** output the full analysis text in the chat window. When all sections are complete, present **only the Hypothesis Check** in the chat for review. This keeps the context window lean and prevents autocompaction from disrupting the analysis mid-flow.
...
- **Context frame:** The preliminary hypothesis and Pass 1 focus questions from the Context step are your analytical entry point. At each flag or anomaly in Part A, note the specific term to grep in footnotes or MD&A — you will execute those searches after completing Part B.
...
All sections below constitute the full Pass 1 output. Every question must be answered. This entire output — Part A through Hypothesis Check — will be committed to `### The Numbers` in the Thesis file.
...
*For each metric, write a concise paragraph synthesizing the current level, trend, and what the data implies. If peer data is provided, compare directly within each metric.*
...
[Answer. Is the operating margin trajectory consistent with AI-driven operating leverage — revenue growth dropping more than proportionally to the bottom line? Because the source of margin improvement (AI productivity versus other factors) cannot be isolated from financial statements alone, cross-reference the Context step findings and flag for targeted MD&A search if the trend is ambiguous.]
...
[Answer. Does the capex pace reflect contracted customer demand — visible in backlog, deferred revenue, or named customer commitments — or speculative buildout ahead of proven demand? Is the stated depreciation schedule realistic given that AI hardware obsolescence is unpredictable? If useful life assumptions appear long relative to peers or industry norms, quantify what a more conservative assumption would imply for reported earnings.]

**6. What does the debt profile tell us about financial risk and the company's ability to service its obligations?**
[Answer. Do the standard leverage ratios appear to capture the full financial exposure? Off-balance-sheet structures, vendor financing arrangements, and contingent commitments tied to AI infrastructure buildout can materially change the true leverage picture — note whether the Accounting Checklist Category 3 findings alter the debt assessment from what the standard ratios show.]
...
[Answer. Are the AI investments generating measurable financial returns? Specific signals to look for: operating margin expanding faster than revenue growth (leverage materializing); deferred revenue growing faster than reported revenue (contracted demand building ahead of recognition); revenue per customer or ASP increasing (pricing power); D&A declining as a percentage of revenue while capex continues (assets being deployed productively). Does the supply chain tier (from the index) support a case that structural demand or pricing power is understated in current reported figures — real returns not yet visible at the consolidated level?]

**9. Is the AI investment cycle self-sustaining?**
Does the operating margin trajectory, combined with the capex and debt picture, suggest the business can fund continued growth from its own cash generation? Or does continued revenue growth require ongoing external capital commitments? Reference MD&A language on the path to self-funding where available, and flag this question for the earnings call in Pass 2 if the financial statements leave it unresolved.

**10. What new questions, concerns, or opportunities do the metrics raise? Note any specific terms to grep in footnotes or MD&A — these feed directly into Targeted Searches.**
[Answer]
...
*This is the question Part A and Part B have been building toward. Answer it explicitly.*

**Is earnings growth real, durable, and sustainable? Does what the company earns justify the price trajectory, and is that trajectory accelerating or decelerating? Is AI-driven earnings growth self-sustaining — or dependent on a continuing investment cycle that hasn't yet been stress-tested? Is there a credible case that AI operating leverage is accelerating in ways not yet visible in current financials?**
[Answer]
...
*Execute after Part B and the Central Question. Driven entirely by flags from Part A — not a predetermined list. `{TICKER}_notes.md` and `{TICKER}_mda.md` are never read in full — only matched lines enter context.*

For each search:

- **Term:** what you are searching for and why Part A flagged it
- **Command:** the grep command run
- **Findings:** matched lines
- **Interpretation:** what the result confirms, disputes, or leaves unresolved

Common terms to consider: goodwill impairment assumptions, revenue recognition policy changes, SBC vesting schedules, off-balance sheet commitments, segment reclassifications, deferred revenue, useful life changes, receivables factoring, non-GAAP definitions, restructuring charges.

Additional terms: "useful life," "variable interest entity," "VIE," "vendor financing," "write-down," "impairment" (AI infrastructure-specific), "related party."

If Part A raised no flags warranting footnote investigation, state that explicitly.
...
State the preliminary hypothesis from Context. Then state explicitly whether the financial picture — Part A, Part B, Central Question, targeted searches, and accounting analysis taken together — confirms, disputes, or complicates it. If the numbers complicate rather than resolve the thesis, name what specifically is unresolved and why.
...
- Has each metric in Part A been assessed across all four dimensions (current vs. historical, long-term trend, short-term trend, metric interpretation)?
- If peer data was provided, has each metric in Part A been compared against peers?
- Do Part B questions draw meaningfully on the per-metric work in Part A?
- Has the Central Question been answered explicitly — not inferred from surrounding text?
- Are all targeted searches driven by specific Part A flags, with each result interpreted?
