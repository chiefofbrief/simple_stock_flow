...
- **GAAP P/E Cross-Check (required):** The P/E used in the Price & Earnings screening phase is derived from earnings press release figures, which for most large-cap technology companies are **non-GAAP** (excluding SBC, amortization, and other add-backs). Compute the GAAP P/E directly from the income statement data: `current_price ÷ (TTM net income ÷ TTM diluted shares)`. Compare this to the screening P/E from the Thesis file. If the two diverge by more than 20%, flag it explicitly in the OCF/Net Income metric and in Part B Question 2. State both figures, identify the primary driver of the gap (typically SBC), and restate the absolute valuation conclusion using the GAAP P/E. A non-GAAP P/E that clears the "reasonable floor" threshold (20–30x) may appear significantly more expensive on a GAAP basis — this changes the thesis risk/reward and must be surfaced, not buried.
