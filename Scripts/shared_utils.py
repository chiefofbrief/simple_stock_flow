...
def parse_tickers_from_tracker(categories):
    """Extract ticker symbols from Stock_Tracker.md matching the given category tags."""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    tracker_path = os.path.join(os.path.dirname(current_dir), "Stock_Tracker.md")
    
    if not os.path.exists(tracker_path):
        tracker_path = "Stock_Tracker.md"
        if not os.path.exists(tracker_path):
            print(f"Error: Stock_Tracker.md not found at {tracker_path}")
            return []
    ...
    for cat in categories:
        tag = tag_map.get(cat.lower())
        if tag:
            target_tags.append(tag)
        else:
            target_tags.append(f"[{cat.upper()}]")
    ...
    for line in lines:
        if line.startswith("|") and not line.startswith("|---"):
            ...
            if len(parts) >= 6:
                ticker = parts[1]
                tags_col = parts[5]
                if any(tag in tags_col for tag in target_tags):
