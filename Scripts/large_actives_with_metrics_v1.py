...
Saves:   Large_Actives_with_Metrics_v1.csv  (repo root)
API key: FMP_API_KEY.  ~60 min (7 calls/symbol + FX).

ARCHIVED — this is the original composite-score version (growth_score /
profitability_score / *_flag columns).  The current approach lives in
large_actives_with_metrics.py (per-axis percentiles + composite).  Kept for
reference; writes its own CSV so the two never clobber each other.
"""
import bisect
import csv
import os
...
OUT_PATH = os.path.join(DATA_DIR, "Large_Actives_with_Metrics_v1.csv")
