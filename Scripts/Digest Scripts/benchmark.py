"""
Benchmark Minerals RSS Feed Fetcher (via Kill the Newsletter)
============================================================

Fetches and displays the latest critical minerals, EV battery, and energy 
storage news from Benchmark Minerals.

Usage:
    python benchmark.py                       # Last 24 hours/same day (default)
    python benchmark.py --days 0              # Today only
    python benchmark.py --markdown            # Output for Peter's Digest
"""
...
RSS_FEED_URL = "https://kill-the-newsletter.com/feeds/wswfc6jtuvcgyehx40pb.xml"
DEFAULT_DAYS = 1
DEFAULT_COUNT = 15
...
def main():
    ...
    if args.markdown:
        print(f"## Benchmark Minerals - Critical Minerals Insights")
        if not articles:
        ...
        console = Console()
        console.print(Panel(Text("Benchmark Minerals", style="bold white on orange"), border_style="bright_yellow"))
        for art in articles:
