#!/usr/bin/env python3
"""
TrendForce News Fetcher (via Perigon)
======================================

Fetches and displays the latest TrendForce news via the Perigon API.

Usage:
    python trendforce.py --markdown
    python trendforce.py --date 2026-04-14 --markdown
"""
...
def fetch_trendforce(api_key, target_date=None, count=DEFAULT_COUNT):
    params = {
        "apiKey": api_key,
...
def main():
    ...
    articles = fetch_trendforce(api_key, target_date=args.date, count=args.count)

    if args.markdown:
        print("## TrendForce - Market Intelligence")
        if not articles:
        ...
        for a in articles:
            ...
            if len(desc) > 500:
                desc = desc[:497] + "..."
            print(f"### {title}")
            print(f"_{pub_date}_ | [Read Article]({url})\n")
            if desc:
                print(f"{desc}\n")
            print("---\n")
    else:
