...
- `Data/tickers/{TICKER}/{TICKER}_notes.md` — Notes to Financial Statements (10-K + 10-Q). Run: `python Scripts/footnotes.py {TICKER}`
- `Data/tickers/{TICKER}/{TICKER}_mda.md` — MD&A sections (10-K Item 7 + 10-Q Item 2). Generated by the same script.
...
If all sections pass, also do a quick content sanity check on `{TICKER}_notes.md` and `{TICKER}_mda.md`:
