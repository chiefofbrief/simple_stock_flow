...
2. **Conditional Check:** Has the correct conditional logic been applied based on the stock's tags in `Stock_Tracker.md`?
...
### Scope Guidelines
- **Two commits required:** Update `Stock_Tracker.md` and append the full analysis output to `Data/screening/Price_Data_{DATE}.txt`.
...
**`Data/screening/Price_Data_{DATE}.txt` — Append Analysis**
...
3. **Next Steps Check:** Does the Next Steps update correctly identify PASS tickers for Earnings screening?
...
- **Commit:** Upon approval, write all updates to `Stock_Tracker.md` and append the full analysis to `Data/screening/Price_Data_{DATE}.txt`.
