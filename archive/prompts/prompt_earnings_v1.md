...
- `Stock_Tracker.md` — The stock's current Tags (e.g., `[LOSER]`, `[TAILWIND]`, `[AI]`).
- `Discovery_Context.md` — The original reason for screening this stock.
...
- For conditional sections, check `Stock_Tracker.md` to confirm the stock's tags, apply the relevant conditional, and refer to the matching example below:
...
3. **Conditional Check:** Has the correct conditional logic been applied based on the stock's tags in `Stock_Tracker.md`?
...
- **Action:** Ask: *"Do you approve this recommendation? Should I update the Stock Tracker and append this analysis to the data file?"*
...
### Scope Guidelines
- **Two commits required:** Update `Stock_Tracker.md` and append the full analysis output to `Data/screening/Earnings_{DATE}.txt`.
...
**`Stock_Tracker.md` — Ticker Dashboard Table**
...
**`Data/screening/Earnings_{DATE}.txt` — Append Analysis**
...
**Questions:**
1. **Scope Check:** Are changes limited strictly to the Ticker Dashboard table, Recent Activity Log, Next Steps, and Data File?
2. **Status Check:** Does the Dashboard update accurately reflect the PASS result — and have FILTERED tickers been removed?
3. **Next Steps Check:** Does the Next Steps update correctly identify PASS tickers for the Screening Completion prompt?
...
- **Commit:** Upon approval, write all updates to `Stock_Tracker.md` and append the full analysis to `Data/screening/Earnings_{DATE}.txt`.
